...
(ns server.twig.page-files
  (:require [recollect.bunch :refer [create-twig]] [clojure.set :as set]))

(defn keys-set [x] (into #{} (keys x)))
...
(defn render-changed-files [files saved-files]
  (->> (set/union (keys-set files) (keys-set saved-files))
       (filter
